...
**✅ 已过手**：[`ex06_map_filter`](../../exercises/ex06_map_filter/)（2026-08-14，**16 条一次全过**）。
两处值得记的观察在 `MASTERY.md` 2026-08-14：
① 他给 `List.map (fun x -> string_of_int x)` 包了层空转的 `fun`，
但第 7 题传参数 `pred` 时又没包 —— **不是不会，是不一致**，用「函数是值」点破后自己改了
（顺带给了 **η-约简** 这个名字）；
② `ocamlformat` 删掉了他多余的括号，**客观回答了他 08-06 问过的「加括号求稳会不会太谨慎」**。

> ⚠️ **本篇是 08-11 讲的那一次的记录，08-14 在笔记本上又讲了一遍**
> （因为 08-11 的提交没及时推上去）。两次内容一致，**切入点都是 `qsort` comparator，都有效**。

**后续**：`fold` 见 C2（2026-08-14 讲完，`ex07_fold`）；
`|>` / `@@` 和运算符全表是 C3 / C3.5；`List` 模块其他函数（`iter` / `exists` /
`for_all` / `mem` / `sort` / `assoc` …）仍未讲。
