...
## 14.7 ⭐⭐ 异常 vs `option`：怎么选

> 2026-08-19 补。**这一节从 2026-08-10 就留着坑**，缺的前置是 `option`（B2），
> 2026-08-18 讲完之后终于能写了。配套：[`20-option.md`](20-option.md)。

### 现成的例子：同一个函数，两种失败，两种处理

```
List.nth_opt [1;2] 5      →  None                          表太短
List.nth_opt [1;2] (-1)   →  Invalid_argument "List.nth"   下标为负（实测）
```

**`nth_opt` 不是「把所有失败都变成 None」** —— 它只把其中一种变成了 `None`。

### 判据：这个失败是不是「这个函数的正常输出之一」

| | 「表里有 2 个，你要第 5 个」 | 「你要第 -1 个」 |
|---|---|---|
| 这个问题**成立吗** | ✅ 成立，答案就是「没有」 | ❌ 问题本身不成立 |
| `None` 是什么意思 | **「我找过了，那里没东西」**——真答案 | **撒谎**——根本不存在「那里」 |
| 谁该处理 | **调用方**（他本来就该考虑「可能没有」） | **写这行代码的人**，改代码 |

> **`option` 表达「查了，没有」；异常表达「你这么问没有意义」。**

第二种如果也返回 `None`，等于**把一个 bug 伪装成了一个正常结果**
—— 和 `atoi("abc")` 返回 0 是同一种病（`20-option.md` 20.2）。

### ⭐ 锚点用**内核的两条错误路径**（他是这方面的专家，一次就通）

| 内核 | 对应 |
|---|---|
| **`return -ENOENT;`** | 正常的失败路径，**调用方必须检查**，天天发生 → `option` |
| **`BUG_ON()` / `WARN_ON()`** | 不变量被破坏，**不该发生**，发生了就是代码错了 → 异常 |

### 📌 落到写代码时的规矩

> **正常情况用 `option`，调用方违约用异常。**
>
> 自检：**「这件事发生了，是我该处理，还是写这行代码的人该改代码？」**

### 他的回答（2026-08-19，质量很高）

> 「机械上两种都是超界……我能想到的唯一可能不同是**两种问题常见的触发背景不同**，
> 负下标可能是索引未初始化或者特意触发某些错误捕获流程，
> 而索引正向超界往往是迭代逻辑中没有正确进行尾部拦截。」

**「触发背景不同」已经踩到要害**，只差把它从「通常怎么发生」推到「是不是正常输出」。
**讲法：先肯定这一句，再把它往前推一步。**

### 顺带：C 里为什么看不到这个区分

```c
arr[i]        /* i 是 size_t 时，-1 变成 SIZE_MAX，两种情况塌成一种 */
```

**C 用 `size_t` 把「负数」从类型上消掉了**，代价是**出事时也分不出是哪种**。
OCaml 的 `int` 是有符号的（63 位），负下标能表示出来，**所以它必须自己决定怎么对待**
—— 它选了「区别对待」。**这是他自己提出「有符号数」之后顺势给的，接得很顺。**
