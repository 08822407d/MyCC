...
- 教到：**第 3 个知识点 `let … in` 作用域 + 同名遮蔽**（2026-08-04 讲完，笔记在
  `notes/concepts/03-let-in-scope.md`）。这一点是**提前讲的**——用户从公开课带问题回来，顺势就讲了。
- **⚠️ 有两件事同时挂着：**
  1. **口算题等用户回答**：`let x = 10 in (let x = x + 1 in x) + x`，
     **答案 21**（已实测），考「等号右边看到的是外层绑定」。**别主动说破。**
  2. **第 2 点的练习一个 TODO 没写**：`exercises/ex01_int_float/main.ml`。
...
- 2026-08-04 是**第一次拿到用户的真实作答数据**，而且表现不错（他主动说出
  「不是覆盖值，是名称覆盖」）。详见 `notes/MASTERY.md`，那里终于不是「暂无数据」了。
- **下次开工第一件事**：先收上面那道口算题的答案，再问 `ex01_int_float` 写了没有。
  写了就直接跑，别问要不要跑。
