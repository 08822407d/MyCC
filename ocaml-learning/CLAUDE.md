...
- 教到：**第 5 个知识点**（2026-08-05）。笔记都在 `notes/concepts/`：
  3 = `let … in` 作用域与遮蔽，4 = **函数是值 / 部分应用 / 多参数其实是嵌套**，5 = 类型转换。
  第 3、4 点都是**提前讲的**——用户从公开课带问题回来，顺势就讲了。
- **⚠️ 有三件事同时挂着：**
  1. **「柯里化」这个词还没说出口。** 机制（`f a b` ≡ `(f a) b`、部分应用）已经讲透，
     只差贴标签。我提议过两次，用户都先去问别的了。**下次可以直接给。**
  2. **口算题始终没收到答案**：`let x = 10 in (let x = x + 1 in x) + x`，
     **答案 21**（已实测），考「等号右边看到的是外层绑定」。**别主动说破。**
  3. **`exercises/ex01_int_float/main.ml` 三个 TODO 挂了三天没写。**
     知识点 5 讲完后**所需知识已全齐**，纯粹是没动手。用户自述懒，**主动催是合理的**。
     他说「好了」就直接跑 `bash ./scripts/ocaml.sh run ex01_int_float`。
...
- **下次开工第一件事**：先收那道口算题的答案，再催 `ex01_int_float`。
  写了就直接跑，别问要不要跑。
