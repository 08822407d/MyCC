...
## `fmt` 不只是排版 —— 它能暴露结构 bug（2026-08-06 实测）

**OCaml 的缩进不参与解析**，编译器根本看不见它。所以「靠缩进看出代码结构」原理上不成立
——嵌套 `match` 那个经典陷阱就是这么来的（详见 `notes/glossary.md` 的「模式匹配」条）。

**但格式化之后缩进就可信了**，因为那时它是**从解析树生成的**，必然反映真实结构。

> **启发式：写完跑一遍 `fmt`，如果缩进跳动很大，说明你以为的结构和真实结构不一致 —— 那是个警报。**

实测：把那个嵌套 `match` 陷阱丢给 ocamlformat，它把三条分支全部缩进到内层，
还自己补上了括号 —— 光看排版 bug 就藏不住了。

⚠️ 注意它加的是**反映当前解析结果**的括号，**不是修 bug 的括号**。
它只负责让真实结构可见，不负责让结构变成你想要的。

只想看某个文件格式化后长什么样、不改文件（**这条是 win10-laptop 的写法**）：

```bash
wsl -d Ubuntu -- bash -lc 'cd /mnt/d/MyCC/ocaml-learning && ocamlformat --enable-outside-detected-project <文件>'
```
